...
def load_original_data(D_name: str) -> pd.DataFrame:
    if D_name == 'chiller':
        path = PROJECT_ROOT / "dataset" / "RP_1043" / "tabular" / "Train" / f"chiller_L1_Train_{select_number}.csv"
        return pd.read_csv(path, sep=',', header='infer')
    if D_name == 'AHU':
        path = PROJECT_ROOT / "dataset" / "RP_1312" / "tabular" / "train" / f"train_{select_number}.csv"
        return pd.read_csv(path, sep=',', header='infer')
