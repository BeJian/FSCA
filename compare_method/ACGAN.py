...
data_dir = PROJECT_ROOT / "datasets" / "image" / "AHU" / "Train" / f"{num}"
loss_dir = PROJECT_ROOT / "generate_data" / "ACGAN" / "AHU" / f"N{num}" / "Runs"
data_aug_dir = PROJECT_ROOT / "generate_data" / "ACGAN" / "AHU" / f"N{num}" / "Augmented"
...
num_classes_total = len(full_dataset.classes)

os.makedirs(loss_dir, exist_ok=True)

class Generator(nn.Module):
    def __init__(self, channels, num_classes, latent_dim=100):
        super().__init__()
        ...
        self.main_module = nn.Sequential(
            nn.ConvTranspose2d(in_channels=latent_dim + num_classes, out_channels=1024, kernel_size=4, stride=1, padding=0),  # 1→4
            nn.BatchNorm2d(1024),
            nn.ReLU(True),
    ...
    def forward(self, z, labels):
        # labels: [bsz] (long)
        bsz = z.size(0)
        label_vec = self.label_emb(labels).view(bsz, self.num_classes, 1, 1)
...
class Discriminator(nn.Module):
    def __init__(self, channels, num_classes):
        ...
        self.aux_head = nn.Conv2d(1024, num_classes, kernel_size=4, stride=1, padding=0)  # 4→1

    def forward(self, x):
        feats = self.main_module(x)
        validity = self.adv_head(feats)                   # [bsz,1,1,1]
        class_logits = self.aux_head(feats)              # [bsz,num_classes,1,1]
        class_logits = class_logits.view(x.size(0), self.num_classes)  # [bsz,num_classes]
...
class ACGAN(object):
    def __init__(self, args, weight_dir: str, sample_dir: str, num_classes: int, class_idx: int):
        print("ACGAN model initialization.")
        self.G = Generator(args.channels, num_classes).to(device)
        self.D = Discriminator(args.channels, num_classes).to(device)
        ...
        self.aux_loss = nn.CrossEntropyLoss().to(device)
    ...
    def train(self, train_loader):
        ...
        for epoch in range(self.epochs):
            ...
            for i, (images, labels) in enumerate(train_loader):
                ...
                real_valid = torch.ones(bsz, device=device)
                fake_valid = torch.zeros(bsz, device=device)

                real_pred, real_cls = self.D(images)               # [bsz,1,1,1], [bsz,num_classes]
                d_loss_real_adv = self.adv_loss(real_pred.view(-1), real_valid)
                d_loss_real_aux = self.aux_loss(real_cls, labels)
                d_loss_real = d_loss_real_adv + d_loss_real_aux

                z = torch.randn(bsz, 100, 1, 1, device=device)
                gen_labels = labels
                fake_images = self.G(z, gen_labels)
                fake_pred, fake_cls = self.D(fake_images.detach())
                d_loss_fake_adv = self.adv_loss(fake_pred.view(-1), fake_valid)
                d_loss_fake_aux = self.aux_loss(fake_cls, gen_labels)
                ...
                gen_labels = labels
                fake_images = self.G(z, gen_labels)
                g_pred, g_cls = self.D(fake_images)
                g_loss_adv = self.adv_loss(g_pred.view(-1), real_valid)
                g_loss_aux = self.aux_loss(g_cls, gen_labels)
                ...
                if generator_iter % 1000 == 0:
                    ...
                    with torch.no_grad():
                        ...
                        fixed_labels = torch.full((800,), self.class_idx, device=device, dtype=torch.long)
                        samples = self.G(z, fixed_labels).mul(0.5).add(0.5)
    ...
    def evaluate(self, bsz: int = None):
        ...
        with torch.no_grad():
            if bsz is None:
                bsz = self.batch_size
            z = torch.randn(bsz, 100, 1, 1, device=device)
            labels = torch.full((bsz,), self.class_idx, device=device, dtype=torch.long)
            samples = self.G(z, labels).mul(0.5).add(0.5).data.cpu()
            grid = utils.make_grid(samples)
            out_path = os.path.join(self.sample_dir, 'acgan_eval.png')
    ...
    def generate_latent_walk(self, number):
        ...
        with torch.no_grad():
            ...
            fixed_label = torch.full((1,), self.class_idx, device=device, dtype=torch.long)
            for _ in range(1, number_int + 1):
                z_intp = z1 * alpha + z2 * (1.0 - alpha)
                alpha += alpha
                fake_im = self.G(z_intp, fixed_label).mul(0.5).add(0.5)  # denormalize
...
if __name__ == "__main__":
    ...
    for class_name in classes:
        ...
        model = ACGAN(args, weight_dir=weight_dir_cls, sample_dir=sample_dir_cls,
                            num_classes=num_classes_total, class_idx=target_idx)
        ...
        with torch.no_grad():
            while done < Au_num_ber:
                ...
                labels = torch.full((cur,), target_idx, device=device, dtype=torch.long)
                gen_imgs = model.G(z, labels).cpu()  # [-1,1]
                for k in range(cur):
