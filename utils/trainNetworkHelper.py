...
class TrainerBase(nn.Module):
    def __init__(self,
        ...
        if self.epoches is None:
            raise ValueError("epoachs")
        self.train_loader = train_loader
        if self.train_loader is None:
            raise ValueError("train_loader")
        ...
        if IFEarlyStopping:
            if "patience" in kwargs.keys():
                ...
                raise ValueError("{patience=int X}")
        ...
        if IFadjust_learning_rate:
            if "types" in kwargs.keys():
                ...
                raise ValueError("{type1 or type2} lr types")
    def adjust_learning_rate(self, epoch, learning_rate):
        if self.types == 'type1':
            ...
            if self.lr_adjust is not None:
                ...
                lr_adjust = {5: 1e-4, 10: 5e-5, 20: 1e-5, 25: 5e-6,
                             30: 1e-6, 35: 5e-7, 40: 1e-8}
        else:
            raise ValueError("{type1 or type2} lr update types")
    ...
    def forward(self, model, *args, **kwargs):
        for i in range(self.epoches):
            ...
            for step, (features, labels) in loop:
                ...
                labels = labels.to(self.device)  # 条件信息
                batch_size = features.shape[0]
                t = torch.randint(0, self.timesteps, (batch_size,), device=self.device).long()
                # 传入条件参数
                loss = model(mode="train", x_start=features, t=t, loss_type="huber", condition=labels)
                losses.append(loss)
            ...
            if self.IFadjust_learning_rate:
                self.adjust_learning_rate(i, self.optimizer.param_groups[0]['lr'])
