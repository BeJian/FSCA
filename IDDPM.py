...
train_num = 20
custom_dataset_path = PROJECT_ROOT / "datasets" / "image" / "AHU" / "Train" / f"{train_num}"
image_size = 64
channels = 1
batch_size = 4
epoches = 10000
    ...
    def __init__(self, root_dir, transform=None):
        ...
        for folder in os.listdir(root_dir):
            ...
            if not os.path.isdir(folder_path):
                continue
            try:
                label = int(folder[1:]) - 1 if folder.startswith("F") else int(folder.split('-')[0])
...
output_base = PROJECT_ROOT / "generate_data" / "DDPM-UNet" / "AHU" / f"N{train_num}" / f"Generate_{epoches}_{U}_{LR}_se2(resnet_block_groups=8)_Summer(20)_U1SEM"
